...
import { clearStateTopRatedMovies2, fetchTopRatedMovies2 } from "../redux/sliceTopRatedMovies";
...
export const TopRatedMovies2 = () => {
    const nav = useNavigate() ;
    const dispatch = useDispatch();
    const page = useRef(1);
    const topRatedMovies = useSelector((state: RootState) => state.reducerTopRatedMovies2);
    const loadMore = useRef<HTMLButtonElement | null>(null);
    useEffect(() => {
        document.title = 'Top Rated Movies'
        dispatch(fetchTopRatedMovies2(page.current));
        page.current = page.current + 1;
        return(() => {
            dispatch(clearStateTopRatedMovies2({})) ;
        })
    }, [])
        ...
        const observer = new IntersectionObserver(
            (entries) => {
                if (entries[0].isIntersecting) {
                    dispatch(fetchTopRatedMovies2(page.current + 1));
                    page.current = page.current + 1;
                }
            }, {
            threshold: 0.01
    ...
    return (
        <div>
            <div className="font-bold text-2xl mb-12">Top Rated Movies</div>
            <div className="flex flex-wrap gap-2 justify-around">
                {
                    topRatedMovies.top_rated.length > 0 && topRatedMovies.top_rated.map((item, index) => <motion.div
            ...
            <div>{topRatedMovies.error}</div>
            {!topRatedMovies.loading && topRatedMovies.top_rated.length > 0 && <Button className="w-full h-12 my-12 font-bold" ref={loadMore}>Load More</Button>}
        </div>)
}
