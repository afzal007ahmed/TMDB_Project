...
import { fetchStreamingMovies } from "./redux/sliceStreamingMovies";
...
const Streaming = () => {
 ...
 const streamingMovies = useSelector(( state : RootState ) => state.reducerStreamingMovies);
 const dispatch = useDispatch() ;
 useEffect(() => {
    dispatch(fetchStreamingMovies({})) ; 
 } ,[])
...
<div className='relative'>
            <div className="flex items-stretch gap-4 overflow-x-auto scrollbar-hide mt-12 min-h-[340px]">
                {
                    streamingMovies.loading && Array.from({ length: 7 }).map((_,index) => <div className="w-[150px] h-[350px]" key={index}>
                        <Skeleton className="w-[150px] h-[220px]" />
                    </div>)
                }
                {!streamingMovies.loading && streamingMovies.streaming && streamingMovies.streaming.results.map((item ,index) => 
                <motion.div
                key={item.id}
