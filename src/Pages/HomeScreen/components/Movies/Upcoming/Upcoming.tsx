...
import { fetchUpcomingMovies } from "./redux/sliceUpcomingMovies";
import { Skeleton } from "@/components/ui/skeleton";
import { buildStyles, CircularProgressbar } from "react-circular-progressbar";
import { dateMapping } from "../../Trending/utils/utils";
import {motion} from 'framer-motion'
import type { Movie } from "./utils/utils";
import routes from "@/RouteManagement/routes";
import { useNavigate } from "react-router-dom";

const Upcoming = () => {
    const dispatch = useDispatch();
    const upcomingMovies = useSelector((state: RootState) => state.reducerUpcomingMovies);
    useEffect(() => {
    ...
    return (
        <div className='relative'>
            <div className="flex items-stretch gap-4 overflow-x-auto scrollbar-hide mt-12 min-h-[340px]">
                {
                    upcomingMovies.loading && Array.from({ length: 7 }).map((_,index) => <div className="w-[150px] h-[350px]" key={index}>
                        <Skeleton className="w-[150px] h-[220px]" />
                    </div>)
                }
                {!upcomingMovies.loading && upcomingMovies.upcoming && upcomingMovies.upcoming.results.map((item:Movie,index) => 
                <motion.div
                key={index}
